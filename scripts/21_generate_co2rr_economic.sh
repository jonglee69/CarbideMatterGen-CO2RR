...
#   MODEL           fine-tuned model dir (default: outputs/finetune_co2rr_economic)
set -euo pipefail

[ -n "${VENV:-}" ] && source "${VENV}/bin/activate"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MODEL="${MODEL:-${ROOT}/outputs/finetune_co2rr_economic}"
GEN_ROOT="${GEN_ROOT:-${ROOT}/outputs/gen_co2rr_economic}"
