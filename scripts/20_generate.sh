...
MODEL="${MODEL:-${ROOT}/outputs/finetune_carbide}"
GEN_ROOT="${GEN_ROOT:-${ROOT}/outputs/gen}"
CKPT_EPOCH="${CKPT_EPOCH:-best}"          # best = epoch=526-loss_val=0.14

# Per-guidance count = N_CHUNKS × NUM_BATCHES × BATCH_SIZE.
# Default: 10 × 100 × 50 = 50,000 per weight  →  100,000 total (w=1.0 + w=2.5).
...
# Target property values (must reference exactly the 8 trained properties).
# Fire dict syntax: no spaces.
