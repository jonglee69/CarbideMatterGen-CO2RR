...
\begin{itemize}[nosep]
\item $*$COOH 해리성 과결합은 \emph{정밀 이완 기하로 이미 확인}되었다(\ref{sec:stagec}절:
Pr$_3$Y$_2$WC$_6$ 활성 면에서 C--O 결합 하나가 3.3~\AA로 끊겨 $*$CO$+*$O로 분해). \tbc{형식적
투영 결정-궤도(COHP/COOP, LOBSTER) 결합 세기 분석은 선택적 보강}
\item 작동 전위($-0.6$~V)에서 활성 면 \emph{평균장 피복률}은 여섯 후보 모두 반응 중간체 $*$COOH로
사실상 포화($\theta_{*\mathrm{COOH}}\!\approx\!0.87$--$1.0$; CeY$_2$ZrWC$_7$가 가장 낮아 온건 결합)이다.
\tbc{측면 상호작용을 포함한 \emph{피복 의존} DFT 부분피복, 반응-확산 국소 pH 모형, 개회로 산화막 형성의
동역학 장벽(NEB)은 추가 모델링 과제}
\item \tbc{순금속 기준을 넘어선 \emph{카바이드} 표면/벌크의 명시적 Pourbaix(격자 안정화 포함) ---
\ref{sec:ree-stability}절의 순금속 용출 상한을 카바이드 생성에너지로 보정}
\end{itemize}
