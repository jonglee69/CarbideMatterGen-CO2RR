...
어닐링하였다. 이들은 아래의 명시적 무질서 표면 연구의 기질이다. 다음 제일원리 결과는 \emph{진행
중}이며 완료 시 반영할 예정이다.
\begin{itemize}[nosep]
\item \tbc{이완된 면/SQS 슈퍼셀에 대한 DFT(QE) 표면 자유에너지 $\dGCO$, $\dGCOOH$, $\dGH$ ---
MLIP 수준 활성 순위와 $\dGCOOH<\dGH$ 선택성을 DFT 수준에서 확인}
\item \tbc{여섯 후보의 DFT 생성/분해 에너지 및 볼록 껍질(convex hull) 안정성 --- 표~\ref{tab:master}에
보고한 잔류 응력 점검을 넘어서}
\item \tbc{각 후보 최적 자리의 완전한 전산 수소 전극 자유에너지 다이어그램과 한계 전위 $U_L$}
\item \tbc{전도 특성(그림~\ref{fig:pdos})을 흡착 디스크립터와 연결하는 $d$-밴드 중심 / 투영
결정-궤도 분석}
\end{itemize}
