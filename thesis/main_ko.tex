...
직접 확인(LOBSTER)}
\item \tbc{수용액 전해질에서 희토류 성분의 산화/용출과 활성점 피독에 대한 표면 $*$O,$*$OH 피복
Pourbaix(\ref{sec:ree-stability}절 프로토콜; 우리 슬랩 파이프라인 재사용)}
\end{itemize}
...
전위·pH 의존성을 정량화한다. 본 계산은 Stage~B/Phase~A 파이프라인($*$O,$*$OH를 흡착자로 추가)을
그대로 재사용하므로 즉시 실행 가능하다.
