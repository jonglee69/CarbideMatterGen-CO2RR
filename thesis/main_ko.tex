...
직접 확인(LOBSTER)}
\item \tbc{활성 면 정지상태(\ref{sec:ree-stability}절, 작동 전위에서 $*$COOH 우세)의 피복률
(부분 피복)·국소 pH 및 개회로 산화막의 동역학적 안정성 정량}
...
표면 자유에너지·$d$-밴드·볼록 껍질 안정성을 DFT로 검증하였고, 특히 희토류 산화 우려에 대해 같은-면
표면 Pourbaix로 \emph{작동 전위에서는 산화 피독이 아니라 반응 중간체 $*$COOH가 표면 정지상태}임을
보였다(위험은 개회로/기동에 국한). 산화 경향은 La 함량과 연동되므로, 산화 위험이 큰 La-부화 조성은
피하고 표면 $*$OH 결합을 \emph{생성 단계의 조건화 속성}으로 추가하여 활성과 내산화성을 공동 최적화하는
처방을 제시하였다. 이 예측을 시험하기 위해 합성, 전기화학 CO\textsubscript{2}RR, 온라인 가스 분석,
\textsuperscript{1}H NMR, in-situ FT-IR로 이루어진 완전한 실험 검증 프로그램을 설계하였으며, 그 결과가
본 연구를 완성할 것이다.
