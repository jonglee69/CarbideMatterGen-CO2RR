...
직접 확인(LOBSTER)}
\item \tbc{여섯 후보의 \emph{활성 면} 기준 같은-면 완전 표면 Pourbaix($*$COOH 포함) 및 피복률
--- 세 후보의 정지상태 $*$COOH 결과(\ref{sec:ree-stability}절)를 활성 면 $*$O/$*$OH DFT로 완결}
...
\emph{DFT 확정과 정지상태(resting state)---정정.} 각 후보의 최강-$*$OH 면에서 청정슬랩·$*$OH·$*$O를
QE 단일점(DFT$/\!/$UMA, $\Gamma$, 60/480~Ry, 24 슬랩; La-rich 조성은 GPU SCF 비수렴으로 CPU 폴백)으로
재계산하였다. \textbf{DFT와 UMA는 $r=0.986$(MAE 0.21~eV)로 강하게 일치}하여 $*$O/$*$OH 결합
자유에너지를 확증한다($\Delta G_{*\mathrm{OH}}(0)=-2.4\!\sim\!-6.7$~eV, $\Delta G_{*\mathrm{O}}(0)=
-0.6\!\sim\!-4.3$~eV; 표면의 큰 산소친화도 확인). 그러나 \emph{작동 중 표면 정지상태}를 판정하려면
경쟁하는 모든 흡착종을 \emph{같은 면}에서, 특히 CO$_2$RR 반응 중간체 $*$COOH를 포함해
($\Delta G_{*\mathrm{COOH}}(U)=\Delta G_{*\mathrm{COOH}}(0)+eU$) 비교해야 한다. $*$OH 면에 주 DFT의
$*$CO/$*$COOH/$*$H가 존재하는 세 후보에 대해 같은-면 완전 Pourbaix를 산출하면(그림~\ref{fig:pourbaix})
\textbf{Ce$_2$Y$_2$ZrWC$_6$·Pr$_3$Y$_2$WC$_6$는 전 작동창에서, CeY$_2$ZrWC$_7$는 $-0.3$~V 이하에서
$*$COOH(반응 중간체)가 정지상태}이며 $*$O/$*$OH가 지배하지 않는다. 즉 \emph{음극 작동 전위에서의
정지상태는 산화 피복이 아니라 $*$COOH 피복(작동 상태)}이다. 강한 $*$OH/$*$O 결합은 산화 \emph{경향}
(개회로·기동 시 위험)을 뜻하지만 작동 전위에서 $*$COOH가 이를 능가한다. 실질 한계는 여러 후보의
\emph{$*$COOH 과결합}($\Delta G_{*\mathrm{COOH}}\!\approx\!-3\!\sim\!-4$~eV $\Rightarrow$ $*$COOH$\to*$CO
강한 흡열, 높은 $U_L$; 표~\ref{tab:che}과 일관)이며 산화 피독과는 다른 문제다. \emph{한계}: 위 분석은
$*$OH 면이 주 DFT 면과 일치하는 세 후보에 국한된다(La$_3$Y$_5$(WC$_3$)$_3$·La$_5$Y$_4$W$_2$C$_9$·
Y$_2$ZrWC$_8$는 활성 면에서의 $*$O/$*$OH DFT가 추가로 필요). 여섯 후보의 활성-면 정지상태 및
피복률 정량은 후속 과제다. 교차 검증은 \ref{ch:experiment}장의 XPS 산화 상태·용출 후분석과
in-situ FT-IR($*$COOH/$*$CO 밴드)로 수행한다.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_co2rr_surface_pourbaix_dft.pdf}
\caption{같은-면 완전 표면 Pourbaix(DFT, $*$COOH 포함): $*$OH 면 데이터가 주 DFT와 일치하는 세
후보. 각 흡착종 $\Delta G$의 전위 의존성. 작동창(음영)에서 최저 $\Delta G$ 종이 정지상태이며, 세
후보 모두 반응 중간체 $*$COOH(녹색)가 우세하여 산화 피복($*$OH/$*$O, 적색)이 지배하지 않는다.}
\label{fig:pourbaix}
\end{figure}
